...
int shooting(char * jpegdata, int size, unsigned int *fb_mem)
{
	// 声明解压缩结构体，以及错误管理结构体
...
{
	// pthread_t tid;
	// pthread_create(&tid, NULL, store_video, NULL);
	...
	// 申请一块适当跟LCD尺寸一样大小的显存
	unsigned int *fb_mem = mmap(NULL, lcdinfo.xres * lcdinfo.yres * lcdinfo.bits_per_pixel/8, //bits_per_pixel/8 每像素色彩位数
